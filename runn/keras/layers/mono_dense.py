...
Davor Runje, Sharath M. Shankaranarayana, “Constrained Monotonic Neural Networks”, 
in Proceedings of the 40th International Conference on Machine Learning, 2023.
Github: https://github.com/airtai/monotonic-nn/tree/main
...
def replace_kernel_using_monotonicity_indicator(
    ...
    layer.kernel = apply_monotonicity_indicator_to_kernel(layer.kernel, monotonicity_indicator)
    try:
        yield
    finally:
...
def _check_convexity_params(
    ...
    ix = [i for i in range(len(monotonicity_indicator)) if is_convex[i] and is_concave[i]]

    if len(ix) > 0:
        raise ValueError(f"Parameters both convex and concave: {[names[i] for i in ix]}")
...
def _create_type_1(
    ...
    *,
    ...
    x, monotonicity_indicator, names = _prepare_mono_input_n_param(inputs, monotonicity_indicator)
    has_convex, has_concave = _check_convexity_params(monotonicity_indicator, is_convex, is_concave, names)
    ...
    *,
    ...
    y = [
        (
            ...
            else (Dense(units=input_units, activation=activation, name=f"dense_{names[i]}"))
        )(x[i])
        for i in range(len(inputs))
    ]

    y = Concatenate(name="preprocessed_features")(y)
    monotonicity_indicator_block: List[int] = sum([[abs(x)] * input_units for x in monotonicity_indicator], [])
